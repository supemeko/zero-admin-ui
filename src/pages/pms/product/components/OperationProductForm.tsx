...
const OperationProductForm: React.FC<CreateFormProps> = (props) => {
    ...
    },
    {
      ...
      content: (
        <ProductAttributeInfo
          ...
          onChangeProductParams={onChangeProductParams}
  ...
  const next = () => {
    ...
    console.log(
      '商品添加参数:',
      JSON.stringify({ ...productParams, ...form.getFieldsValue(true) }),
    );
    setCurrent(current + 1);
    setProductParams({ ...productParams, ...form.getFieldsValue(true) });
  };
      ...
      <>
        ...
        <div style={{ marginTop: 24, textAlign: 'center' }}>
          <ProductStepInfo
            current={current}
            steps={steps}
            handleSubmit={() => handleFinish(productParams || {})}
            prev={prev}
            next={next}
          />
        </div>
