...
const ProductAttributeInfo: React.FC<BaseInfoProps> = (props) => {
  const [attributeCategoryListItem, setAttributeCategoryListItem] = useState<
    AttributeCategoryListItem[]
  >([]);
  ...
  const list = props.currentData?.pic?.split(',').map(
    (url: string) =>
      ({
        uid: Math.random() + '',
        name: 'image.png',
  ...
  return (
    <>
      ...
      <FormItem name="name1" label="商品规格">
        <Card>
          {attributeListItem0.map(
            (r) =>
              r.inputList !== '' && (
                <FormItem name={r.id} key={r.id} label={r.name}>
                  <Checkbox.Group key={r.name}>
                    {r.inputList.split(',').map((x) => (
      ...
      <FormItem name="name2" label="商品参数">
        <Card>
          {attributeListItem1.map((r) => (
            <FormItem key={r.id} name="subTitle" label={r.name}>
              {r.inputType === 0 && <Input id={r.name} />}
              {r.inputType === 1 && (
                <Select id="productAttributeCategoryId" placeholder={'请选择' + r.name}>
                  {r.inputList.split(',').map((x) => (
