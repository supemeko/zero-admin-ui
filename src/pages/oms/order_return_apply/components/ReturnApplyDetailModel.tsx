...
const ReturnApplyDetailModel: React.FC<UpdateFormProps> = (props) => {
  ...
  const { detailModalVisible, currentData, onCancel, getCompanyAddressId } = props;
  ...
  const renderContent = () => {
    return (
      <>
        ...
        <Card title="服务单信息">
          <Card type="inner">
            <ReturnApplyBase currentData={currentData} />
          </Card>
          <Card style={{ marginTop: 16 }} type="inner">
            <ReturnApplyInfo
              currentData={currentData}
              addressListItem={companyAddressListItem}
              getCompanyAddressId={getCompanyAddressId}
            />
          </Card>
          {currentData.status === 0 && (
            <Card style={{ marginTop: 16 }} type="inner">
              <ReturnApplyInfoSubmit onSubmit={props.onSubmit} currentData={currentData} />
            </Card>
          )}
          {currentData.status !== 0 && (
            <Card style={{ marginTop: 16 }} type="inner">
              <ReturnApplyHandler currentData={currentData} />
            </Card>
          )}
          {currentData.status === 1 && (
            <Card style={{ marginTop: 16 }} type="inner">
              <ReturnApplyHandlerSubmit onSubmit={props.onSubmit} currentData={currentData} />
            </Card>
          )}
          {currentData.status === 2 && (
            <Card style={{ marginTop: 16 }} type="inner">
              <ReturnApplyReceive currentData={currentData} />
            </Card>
          )}
        </Card>
      </>
