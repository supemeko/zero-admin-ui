...
const OrderDetailModel: React.FC<UpdateFormProps> = (props) => {
  ...
  const { onCancel, onRefresh, updateModalVisible, currentData } = props;
        ...
        <Space direction="vertical" size="large" style={{ display: 'flex' }}>
                ...
                {currentData.status === 1 && (
                  <>
                    ...
                    <Button
                      ...
                      style={{ background: '#06b0ff', color: 'white' }}
                    >
                      取消订单
                    </Button>
                  </>
                )}
                {(currentData.status === 2 || currentData.status === 3) && (
                  <Button
                    type={'primary'}
                    icon={<EditOutlined />}
                    onClick={() => {
          ...
          >
            ...
            <Card style={{ marginTop: 16 }} type="inner" title="商品信息">
              <ProductInfo
                currentData={currentData.listOrderItemData}
